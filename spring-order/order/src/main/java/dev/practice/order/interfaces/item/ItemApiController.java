...
public class ItemApiController {
    ...
    @PostMapping("/change-status-to-on-sales")
    ...
    @PostMapping("/change-status-to-end-sales")
    ...
    public CommonResponse retrieve(@PathVariable("itemToken") String itemToken) {
        var itemDocument = itemFacade.retrieveItemDocument(itemToken);
        var response = itemDtoMapper.of(itemDocument);
        return CommonResponse.success(response);
    }
}
